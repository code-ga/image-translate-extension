...
function tryCanvasBase64(img: HTMLImageElement): Promise<string | null> {
  return new Promise<string | null>((resolve) => {
    try {
      const isCrossOrigin = new URL(resolveBestImageUrl(img)).origin !== window.location.origin;
      if (isCrossOrigin) {
...
function getOrCreateOverlayContainer(): HTMLElement {
  ...
  if (!container) {
    ...
    container.style.cssText = "position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:2147483647;overflow:hidden;";
    document.body.appendChild(container);
  }
  return container;
}

function updateOverlayPosition(img: HTMLImageElement, overlay: HTMLElement) {
...
function addOcrBoxes(overlay: HTMLElement, img: HTMLImageElement, ocrData: OCRResult) {
  ocrData.forEach(box => {
    const fontSize =
      Math.max(
        10,
        Math.min(box.height, box.width) * box.width / img.naturalWidth * 0.7
      );
...
function collectImageInfo(): { src: string; currentSrc: string; width: number; height: number; status: string }[] {
  const images: { src: string; currentSrc: string; width: number; height: number; status: string }[] = []
  for (const img of document.images) {
    ...
    images.push({
      ...
      width,
      height,
      status: processingImages.has(img) ? 'processing' : (processedImages.has(img) && processedImages.get(img) === (img.currentSrc || img.src) ? 'done' : 'pending')
    })
  }
  return images
}
...
async function handleTranslateImages(urls: string[]) {
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    if (i < total - 1) {
      ...
      chrome.runtime.sendMessage<CompleteMessageType>({
...
function collectCanvasInfo(): { index: number; width: number; height: number; status: string }[] {
  const canvases: { index: number; width: number; height: number; status: string }[] = []
  for (let i = 0; i < document.querySelectorAll('canvas').length; i++) {
    ...
    canvases.push({
      ...
      width,
      height,
      status: processingCanvases.has(canvas) ? 'processing' : (processedCanvases.has(canvas) && processedCanvases.get(canvas) === canvas.toDataURL() ? 'done' : 'pending')
    })
  }
  return canvases
}
...
function updateCanvasOverlayPosition(canvas: HTMLCanvasElement, overlay: HTMLElement) {
...
function addOcrBoxesForCanvas(overlay: HTMLElement, canvas: HTMLCanvasElement, ocrData: OCRResult) {
  ocrData.forEach(box => {
    const fontSize =
      Math.max(
        10,
...
function renderCanvasOcrOverlays(canvas: HTMLCanvasElement, ocrData: OCRResult) {
  const canvasKey = canvas.toDataURL()

  if (processedCanvases.has(canvas) && processedCanvases.get(canvas) === canvasKey) {
