...
type ImageInfo = { src: string; currentSrc: string; width: number; height: number; status: string }
type CanvasInfo = { index: number; width: number; height: number; status: string }
...
function App() {
  ...
  const loadSettings = useCallback(async () => {
    const result = await chrome.storage.sync.get(STORAGE_KEY) as Record<string, any>
    const settings = result[STORAGE_KEY] || { enabledDomains: [], enabled: true }
    setEnabledDomains(settings.enabledDomains || [])
    setGlobalEnabled(settings.enabled ?? true)
  }, [])

  const saveSettings = useCallback(async (updates: { enabledDomains?: string[]; enabled?: boolean }) => {
    const current = await chrome.storage.sync.get(STORAGE_KEY) as Record<string, any>
    const settings = { ...(current[STORAGE_KEY] || { enabledDomains: [], enabled: true }), ...updates }
    ...
    chrome.runtime.sendMessage({
      ...
      settings: { enabled: settings.enabled ?? true, enabledDomains: settings.enabledDomains || [] }
    }).catch(() => {})
  }, [])
  ...
  const pollPageData = useCallback(async () => {
    try {
      ...
      try {
        ...
        if (imagesResponse && (imagesResponse as any).type === 'image-status-list') {
          setImages((imagesResponse as any).images)
          setImageCount((imagesResponse as any).images.length)
          setImageProcessingCount((imagesResponse as any).images.filter((i: ImageInfo) => i.status === 'processing').length)
        }
        if (canvasesResponse && (canvasesResponse as any).type === 'canvas-status-list') {
          setCanvases((canvasesResponse as any).canvases)
          setCanvasCount((canvasesResponse as any).canvases.length)
          setCanvasProcessingCount((canvasesResponse as any).canvases.filter((c: CanvasInfo) => c.status === 'processing').length)
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const progressListener = (msg: any) => {
      if (msg.type === 'translate-images-complete' || msg.type === 'translate-images-progress') {
        pollPageData()
      }
    }
  ...
  const isDomainAllowed = (domain: string): boolean => {
    ...
    return enabledDomains.some(d => domain === d || domain.endsWith('.' + d))
  }
  ...
  const addDomain = async () => {
    ...
    await saveSettings({ enabledDomains: [...enabledDomains, domain] })
    setDomainInput('')
  }

  const removeDomain = async (domain: string) => {
    await saveSettings({ enabledDomains: enabledDomains.filter(d => d !== domain) })
  }
    ...
    <div className="popup">
      ...
      <div className="tabs">
        <button className={`tab ${tab === 'images' ? 'active' : ''}`} onClick={() => setTab('images')}>
          Images
          {imageProcessingCount > 0 && <span className="tab-badge processing">{imageProcessingCount}</span>}
        </button>
        <button className={`tab ${tab === 'canvases' ? 'active' : ''}`} onClick={() => setTab('canvases')}>
          Canvases
          {canvasProcessingCount > 0 && <span className="tab-badge processing">{canvasProcessingCount}</span>}
        </button>
            ...
            images.map((img) => (
              <div key={img.currentSrc} className="list-item">
                <div className="item-info">
                  <div className="item-url" title={img.currentSrc}>{img.currentSrc}</div>
                  <div className="item-meta">{img.width}x{img.height}</div>
                </div>
                {img.status !== 'pending' && (
                  <span className={`status-badge ${getStatusClass(img.status)}`}>
            ...
            canvases.map((canvas) => (
              <div key={canvas.index} className="list-item">
                <div className="item-info">
                  ...
                  <div className="item-meta">{canvas.width}x{canvas.height}</div>
                </div>
                {canvas.status !== 'pending' && (
                  <span className={`status-badge ${getStatusClass(canvas.status)}`}>
        ...
        <div className="settings">
          ...
          <div className="setting-item">
            <div className="setting-label">
              ...
              <span className="setting-desc">Turn on/off translation for all pages</span>
            </div>
            <button 
            ...
            <button 
              ...
              onClick={async () => {
                ...
                if (enabledDomains.includes(currentDomain)) {
                  ...
                  await saveSettings({ enabledDomains: [...enabledDomains, currentDomain] })
                }
              }}
              disabled={!currentDomain}
            >
              {enabledDomains.includes(currentDomain) ? 'Remove current domain' : 'Add current domain'}
            </button>
          </div>
          ...
          <div className="domain-list">
            {enabledDomains.map(domain => (
              <div key={domain} className="domain-item">
                ...
                <button className="domain-remove" onClick={() => removeDomain(domain)} title="Remove">Remove</button>
              </div>
            ))}
          </div>
          ...
          <div className="add-domain">
            ...
            />
            <button onClick={addDomain} className="primary">Add</button>
          </div>
          
          {enabledDomains.length > 0 && (
            <button className="clear-all" onClick={() => saveSettings({ enabledDomains: [] })}>
              Clear All Domains
            </button>
          )}
